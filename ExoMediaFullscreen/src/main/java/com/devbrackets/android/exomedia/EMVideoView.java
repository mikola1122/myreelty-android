...
public class EMVideoView extends RelativeLayout implements AudioCapabilitiesReceiver.Listener, VideoTextureView.OnSizeChangeListener {
    ...
    private void readAttributes(Context context, @Nullable AttributeSet attrs) {
        ...
        setDefaultControlsEnabled(enableDefaultControls, false);
     ...
     */
    public void setDefaultControlsEnabled(boolean enabled, boolean isFullScreen) {
        if (defaultControls == null && enabled) {
            ...
            defaultControls.setBus(bus);
            defaultControls.updateFullscreenImage(isFullScreen);
     ...
     * {@link #setDefaultControlsEnabled(boolean, boolean)}.
     */
     ...
     * {@link #setDefaultControlsEnabled(boolean, boolean)}.
     * {@link #setDefaultControlsEnabled(boolean, boolean)} must be called prior to this.
     * <p>
    ...
    /**
     * Sets the EMVideoViewControlsCallback to be used.  {@link #setDefaultControlsEnabled(boolean, boolean)} must
     ...
     * {@link #setDefaultControlsEnabled(boolean, boolean)} must
     * be called prior to this.
     *
    ...
    public void setFullscreenImages(@DrawableRes int fullscreenOnResourceId, @DrawableRes int fullscreenOffResourceId) {
        if (defaultControls != null) {
            defaultControls.setFullscreenImages(fullscreenOnResourceId, fullscreenOffResourceId);
        }
    }
    ...
    /**
     * Returns if a view is currently in fullscreen mode
     *
     * @return True if a view is on fullscreen mode
     */
    ...
    /**
     * Starts the fullscreen mode for the video specified in {@link #setVideoURI(android.net.Uri)}
     * or {@link #setVideoPath(String)}.  This should be called after the VideoView is correctly
     * prepared (see {@link #setOnPreparedListener(android.media.MediaPlayer.OnPreparedListener)})
     */
    public void fullscreenOn() {
        ...
        if (defaultControls != null) {
            defaultControls.updateFullscreenImage(true);
            defaultControls.hideDelayed(DefaultControls.DEFAULT_CONTROL_HIDE_DELAY);
        }
        playRequested = true;
        startProgressPoll();
    ...
    /**
     * If a video is currently in fullscreen mode, it will be paused and the progressPoll
     * will be stopped (see {@link #startProgressPoll(EMEventBus)})
     */
    public void fullscreenOff() {
        if (defaultControls != null) {
            defaultControls.updateFullscreenImage(false);
            defaultControls.hideDelayed(DefaultControls.DEFAULT_CONTROL_HIDE_DELAY);
        }
        playRequested = false;
        stopProgressPoll();
