...
import com.devbrackets.android.exomedia.event.EMMediaFullsceenEvent;
...
public abstract class DefaultControls extends RelativeLayout {
    ...
    protected static final long CONTROL_VISIBILITY_ANIMATION_LENGTH = 100;
    ...
    protected Drawable defaultFullscreenOnDrawable;
    protected Drawable defaultFullscreenOffDrawable;
    protected Drawable defaultPreviousDrawable;
    protected Drawable defaultNextDrawable;

    //Since the Play/Pause and Fullscreen buttons uses 2 separate resource Id's we need to store them
    protected int playResourceId = INVALID_RESOURCE_ID;
    protected int pauseResourceId = INVALID_RESOURCE_ID;
    protected int fullscreenOnResourceId = INVALID_RESOURCE_ID;
    protected int fullscreenOffResourceId = INVALID_RESOURCE_ID;
    ...
    /**
     * Sets the resource id's to use for the fullscreen button.
     *
     * @param fullscreenOnResourceId  The resourceId or 0
     * @param fullscreenOffResourceId The resourceId or 0
     */
    public void setFullscreenImages(@DrawableRes int fullscreenOnResourceId, @DrawableRes int fullscreenOffResourceId) {
        this.fullscreenOnResourceId = fullscreenOnResourceId;
        this.fullscreenOffResourceId = fullscreenOffResourceId;

        updateFullscreenImage(videoView != null && videoView.isFullscreen());
    }
    ...
    /**
     * Makes sure the fullscreen button represents the correct playback state
     *
     * @param fullscreen If the view is currently on fullscreen mode
     */
    public void updateFullscreenImage(boolean fullscreen){
        if (fullscreen) {
            if (fullscreenOffResourceId != INVALID_RESOURCE_ID) {
                fullscreenButton.setImageResource(fullscreenOffResourceId);
            } else {
                fullscreenButton.setImageDrawable(defaultFullscreenOffDrawable);
            }
        } else {
            if (fullscreenOnResourceId != INVALID_RESOURCE_ID) {
                fullscreenButton.setImageResource(fullscreenOnResourceId);
            } else {
                fullscreenButton.setImageDrawable(defaultFullscreenOnDrawable);
            }
        }
    }
    ...
    protected void retrieveViews() {
        ...
        fullscreenButton = (ImageButton) findViewById(R.id.exomedia_controls_full_screen_btn);
    ...
    protected void updateButtonDrawables() {
        ...
        defaultFullscreenOnDrawable = EMResourceUtil.tintList(getContext(), R.drawable.exomedia_ic_fullscreen_white, R.color.exomedia_default_controls_button_selector);

        defaultFullscreenOffDrawable = EMResourceUtil.tintList(getContext(), R.drawable.exomedia_ic_fullscreen_exit_white, R.color.exomedia_default_controls_button_selector);
        fullscreenButton.setImageDrawable(defaultFullscreenOnDrawable);
    ...
    /**
     * Performs the functionality to inform any listeners that the fullscreen
    ...
    protected void onFullscreenClick(){
        ...
        if (bus != null) {
            bus.post(new EMMediaFullsceenEvent());
        }

        //toggles the fullscreen mode
        boolean fullscreen = videoView.isFullscreen();
        if (fullscreen) {
            videoView.fullscreenOff();
        } else {
            videoView.fullscreenOn();
        }
    }
